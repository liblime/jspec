...
(function($, $$){
  ...
  $$.addMatchers({
    ...
    have_classes : function(actual) {
      for (i = 1; i < arguments.length; i++)
        if (!$(actual).hasClass(arguments[i])) return false
      return true
    },
    
    have_attr : function(actual, attr, value) {
      return value ? $(actual).attr(attr) == value:
                     $(actual).attr(attr)
    }
  })
  ...
  $$.each('disabled selected checked', function(attr){
    $$.addMatcher('be_' + attr, "jQuery(actual).attr('" + attr + "')")
  })
  
  // --- have_ATTR
  
  $$.each('type id title alt href src rel rev name target', function(attr){
    $$.addMatcher('have_' + attr, function(actual, value) {
  ...
  $$.each('checkbox radio file password submit image text reset button', function(type){
    $$.addMatcher('be_a_' + type + '_input', function(actual, expected) {
      console.warn("be_a_" + type + "_input is deprected; use have_type('" + type + "')");
      return $(actual).get(0).type == type
    })
  })
