  ...
  JSpec = {
    ...
    JSON : {
      ...
      encode : function(object) {
        ...
        switch (object.constructor) {
          ...
          case String: return JSpec.JSON.escapable.test(object) ?
            '"' + object.replace(JSpec.JSON.escapable, function (a) {
              return typeof JSpec.JSON.meta[a] === 'string' ? JSpec.JSON.meta[a] :
                '\\u' + ('0000' + a.charCodeAt(0).toString(16)).slice(-4)
            }) + '"' :
            '"' + object + '"'
          case Array:  return '[' + map(object, function(val){
            return JSpec.JSON.encode(val)
          }).join(', ') + ']'
          case Object: return '{' + map(object, function(key, val){
            return JSpec.JSON.encode(key) + ':' + JSpec.JSON.encode(val)
          }).join(', ') + '}'
          default: return 'null'
        }
      }
    ...
    load : function(file, callback) {
      ...
      if ('readFile' in main)
        ...
        request.send(null)
        if (request.readyState == 4 && parseInt(request.status.toString()[0]) == 2) 
