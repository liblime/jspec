...
(function(){
  ...
  var JSpec = {
    ...
    ConsoleFormatter : function(results, options) {
      console.info('')
      console.info('Passes: ' + results.stats.passes + ' Failures: ' + results.stats.failures)
      results.each(results.suites, function(description, suite){
        if (suite.ran) {
          ...
          results.each(suite.specs, function(spec){
            ...
            if (spec.requiresImplementation()) { 
              ...
              console.info(spec.description + ' ' + assertionCount)
            }
            else {
              console.error(spec.description + ' ' + assertionCount + ', ' + spec.failure().message)
            }
          })
          console.groupEnd()
        }
     ...
     */
     ...
     TerminalFormatter : function(results, options) {
       // TODO: me!
     },
  ...
  JSpec.formatter = JSpec.ConsoleFormatter
  this.JSpec = JSpec
