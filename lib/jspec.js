  ...
  JSpec = {
    ...
    Suite : function(description, body) {
      ...
      extend(this, {
        ...
        // Add a nested shared behavior

        addSharedBehavior : function(description, body) {
          var suite = new JSpec.Suite(description, body)					
          this.sharedBehaviors.push(suite)
          suite.suite = this
        },
        ...
        // Check if nested shared behaviors are present

        hasSharedBehaviors : function() {
          return this.sharedBehaviors.length  
        },
        ...
        after_each : function(body) {
          return JSpec.currentSuite.addHook('after_each', body)
				...
				shared_behaviors_for : function(description, body){
				  return JSpec.currentSuite.addSharedBehavior(description, body)
				},
    ...
    findSuite : function(description) {
      return find(JSpec.allSuites, function(suite){
        return suite.name == description || suite.description == description
      })
    },

    /**
     * Find a shared example suite by its description or name. First searches parent tree of
		 * suites for shared behavior before falling back to global scoped nested behaviors.
     *
     * @param  {string} description
     * @return {Suite}
     * @api private
    ...
    findSharedBehavior : function(description) {
			return (sharedSuite = JSpec.findLocalSharedBehavior(description)) ? sharedSuite 
																																	: JSpec.findGlobalSharedBehavior(description)
    },

    /**
     * Find a shared example suite within the current suite's parent tree by its description or name.
     *
     * @param  {string} description
     * @return {Suite}
     * @api private
     */
		findLocalSharedBehavior : function(description){
			var currentSuite = JSpec.currentSuite.suite
			
			while( currentSuite ){
				sharedSuite = find(currentSuite.sharedBehaviors, function(suite){
					return suite.name == description || suite.description == description					
				})
				
				if (sharedSuite) return sharedSuite
    ...
    /**
     * Find a shared example suite within the global scope by its description or name.
     *
     * @param  {string} description
     * @return {Suite}
     * @api private
     */
	   ...
	   return find(JSpec.sharedBehaviors, function(suite){
        return suite.name == description || suite.description == description
      })		
		},
    ...
    shareBehaviorsOf : function(description) {
      if (suite = JSpec.findSharedBehavior(description)) {
				JSpec.copySpecs(suite, this.currentSuite)
			}
      else throw 'failed to share behaviors. ' + puts(description) + ' is not a valid shared behavior name'
    },
    ...
    evalBody : function(body, errorMessage, suite) {
    ...
    /**
     * Eval bodies of all suites building up the suite heirarchy.
     *
     * @api public
     */
		...
		evalSuites : function()
		{
			each(this.sharedBehaviors, function(suite) { JSpec.evalSuite(suite) })
			each(this.suites, function(suite) { JSpec.evalSuite(suite) })
		},
		
		/**
     * Eval the body of a suite and it's sub-suites
     *
     * @param  {Suite} suite
     * @api public
     */

    evalSuite : function(suite) {
      this.currentSuite = suite
      this.evalBody(suite.body)

      if (suite.hasSharedBehaviors()) {
        each(suite.sharedBehaviors, function(suite) {
          JSpec.evalSuite(suite)
        })
      }
