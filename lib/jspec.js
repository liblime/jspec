  ...
  JSpec = {
    ...
    jsonencode : function(object) {
      switch (object.constructor) {
        ...
        case Number: return object
        case Array: return '[' + map(object, function(val){
          return JSpec.jsonencode(val)
        }).join(', ') + ']'
        case Object: return '{' + map(object, function(key, val){
          return JSpec.jsonencode(key) + ':' + JSpec.jsonencode(val)
        }).join(', ') + '}'
        default: return 'null'
      }
    },
    ...
    post : function(uri, data) {
      ...
      request.send(JSpec.jsonencode(data))
    },

    /**
     * Report to server with statistics.
     * 
     * When _uri_ is not present the following is used:
     * 'http://' + window.location.host + '/results'
     *
     * @param  {string} uri
     * @api private
     */
    
    reportToServer : function(uri) {
      uri = uri || 'http://' + window.location.host + '/results'
      if (any(hook('reportingToServer', uri), haveStopped)) return
      JSpec.post(uri, JSpec.stats)
