...
(function(){
  ...
  JSpec = {
    ...
    Matcher : function (name, matcher, actual, expected, negate) {
      self = this
      matcher = JSpec.normalizeMatcherBody(matcher)
     ...
     * the matcher body is in its final normalized hash state.
     *
    ...
    normalizeMatcherBody : function(matcher) {
      switch (matcher.constructor) {
        case String:
          if (captures = matcher.match(/^alias (\w+)/)) return JSpec.matchers[last(captures)]
          body = matcher.length < 4 ? 'actual ' + matcher + ' expected' : matcher
    ...
    addMatcher : function(name, body) {
      this.matchers[name] = this.normalizeMatcherBody(body)
      return this
    },
