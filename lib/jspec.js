...
(function(){
  ...
  var JSpec = {
    ...
    Suite : function(description, body) {
      ...
      this.addSpec = function(description, body) {
        this.specs.push(new JSpec.Spec(description, body))
        spec.suite = this
      }
    ...
    preProcess : function(input) {
      ...
      input = input.replace(/describe (.*?)$/gm, 'this.addSuite(JSpec.Suite($1, function(){')
      // Allow it spec literal
      input = input.replace(/it (.*?)$/gm, 'this.addSpec($1, function(){')
      // Allow before / after hooks
      input = input.replace(/(before(?:_each)?|after(?:_each)?)/gm, 'this.addHook("$1", function(){')
      // Allow end }) literal
    ...
    exec : function(file) {
      this.preProcess(this.load(file))
      return this
    }
  }
  ...
  this.addSuite = function(suite) { JSpec.suites.push(suite) }
  this.JSpec = JSpec
