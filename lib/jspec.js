...
(function(){
  ...
  JSpec = {
    ...
    formatters : {
       ...
       *
       * Options:
       *   - color  Utilizes ansi escape sequences for terminal colors when enabled 
       *
       * @api public
       */
       
       Terminal : function(results, options) {
         puts("\n" + color('Passes: ', 'green') + results.stats.passes + 
               " " + color('Failures: ', 'red') + results.stats.failures + "\n")
           ...
           if (suite.ran) {
             ...
             results.each(suite.specs, function(spec){
               ...
               if (spec.requiresImplementation())
                 puts(color(assertionCount + ' ' + spec.description, 'blue'))
               else if (spec.passed())
                 puts(color(assertionCount + ' ' + spec.description, 'green'))
               else 
                 puts(color(assertionCount + ' ' + spec.description + ', ' + spec.failure().message, 'red'))
             })
             puts("\n")
           }          
         }
    ...
    Assertion : function(matcher, actual, expected, negate) {
      extend(this, {
        ...
        exec : function() {
          ...
          if (!this.passed) this.message = matcher.message(actual, expected, negate)
          return this
        }
      })
    },
    ...
    defaultMatcherMessage : function(actual, expected, negate) {
