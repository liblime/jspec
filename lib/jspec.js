  ...
  JSpec = {
    ...
    ProxyAssertion : function(object, method, times, negate) {
      ...
      extend(this, {
        ...
        run : function() {
          var methodString = 'expected ' + object.toString() + '.' + method + '()' + (negate ? ' not to ' : '' )
          ...
          if (this.expectedResult && (negate ? this.anyResultsPass() : this.anyResultsFail()))
            this.message = methodString + ' to return ' + puts(this.expectedResult) + 
              ' but got ' + puts(negate ? this.passingResult() : this.failingResult())

          if (this.expectedArgs && (negate ? this.anyArgsPass() : this.anyArgsFail()))
            this.message = methodString + ' to be called with ' + puts.apply(this, this.expectedArgs) +
             ' but was' + (negate ? '' : ' called with ' + puts.apply(this, this.failingArgs()))

          if (negate ? this.calls.length > this.times : this.calls.length < this.times)
