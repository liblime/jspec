...
var JSpec = {
  ...
  /**
   * Ensure all async requests have completed and
   * JSpec is ready for reporting, aka not paused.
   *
   * @param  {function} callback
   * @api private
   */
  ...
  ready : function(callback) {
    this.readyCallback = this.readyCallback || callback
    this.timePassed += this.asyncInterval
    if (this.timePassed < this.asyncTimeout) {
      if (this.isPaused())
        setTimeout('JSpec.ready()', this.asyncInterval)
      else
        this.readyCallback()
    }
    else {
      this.throw("Async timeout in spec '" + this.currentSpec.description + "'. Exceeded " + this.asyncTimeout + ' milliseconds')
    }
  },
  
  /**
   * Weither or not JSpec is paused.
   *
   * @return {bool}
   * @api public
   */
