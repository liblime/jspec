  ...
  var JSpec = {
    ...
    Matcher : function (name, matcher, actual, expected, negate) {
      ...
      function generateMessage() {
        return 'expected ' + print(actual) + ' to ' + (negate ? ' not ' : '') + name.replace(/_/g, ' ') + ' ' + print(expected[1])
      }
    ...
    hash : function(object) {
      return inject(object, '', function(buffer, key, value){
        if (value.constructor == Array || value.constructor == Object ) buffer += hash(value)
        else buffer += key.toString() + value.toString()
        return buffer      
      })
    },
    ...
    query : function(key, query_string) {
      ...
      return inject(query_string.split('&'), null, function(value, pair){
        parts = pair.split('=')
        k = parts[0] || ''
        v = parts[1] || ''
        return  k == key ? v.replace(/%20|\+/gmi, ' ') : value
      })
    },
