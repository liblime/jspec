  ...
  var JSpec = {
    ...
    ProxyAssertion : function(object, method, times) {
      ...
      extend(this, {
        ...
        run : function() {
          methodString = object.toString() + '.' + method + '()'
          if (this.calls.length < this.times)
            this.message = 'expected ' + methodString + ' to be called ' + this.times + ' time(s), but ' + 
              (this.calls.length == 0 ? ' was not called' : ' was called ' + this.calls.length + ' time(s)')
          if (this.expectedResult && this.anyResultsFail())
            this.message = 'expected ' + methodString + ' to return ' + print(this.expectedResult) + 
              ' but got ' + print(this.failingResult())
          if (this.expectedArgs && this.anyArgsFail())
            this.message = 'expected ' + methodString + ' to be called with ' + 
              print.apply(this, this.expectedArgs) + ' but was called with ' +
                print.apply(this, this.failingArgs())
