...
(function(){
  ...
  var JSpec = {
    ...
    defaultContext : {
      ...
      an_instance_of : function(object) {
        return { an_instance_of : object }
      }
    },
    ...
    Assertion : function(matcher, actual, expected, negate) {
      extend(this, {
        ...
        exec : function() {
          // TODO: rename to run() (ProxyAssertion as well)
    ...
    ProxyAssertion : function(object, method, times) {
      ...
      object[method] = function(){
        args = []
        // TODO: refactor ... this is done a few times make a util
      ...
      extend(this, {
        ...
        with_args : function() {
          // TODO refactor..
          for (i = 0; i < arguments.length; i++) this.args.push(arguments[i])
        },
        
        anyResultsFail : function() {
          return any(this.calls, function(call){
            return hash(self.expectedResult) != hash(call.result)
          })
        },
        
        failingResult : function() {
          // TODO: create find() util
          return inject(this.calls, null, function(result, call){
            return hash(self.expectedResult) != hash(call.result) && !result ?
                     result = call.result:
                       result = result
          })
        },
        ...
        anyArgsFail : function() {
          return any(this.calls, function(call){
            return hash(self.args) != hash(call.args)
          })
        },
        
        failingArgs : function() {
          // TODO: refactor / renamed ... convetions keep shifting expectedArgs etc
          return inject(this.calls, null, function(args, call){
            return hash(self.args) != hash(call.args) && !args ?
                     args = call.args:
                       args = args
          })
        },
        ...
        exec : function() {
          if (typeof object[method] != 'function')
            this.message = object.toString() + ' does not respond to ' + method
          else if (this.calls.length < this.times)
            this.message = object.toString() + '.' + method + '() was called ' + this.calls.length + ' times, expected ' + this.times
          else if (this.expectedResult && this.anyResultsFail())
            this.message = object.toString() + '.' + method + '() did not return the expected result of ' + print(this.expectedResult) + '; got ' + print(this.failingResult())
          else if (this.args.length && this.anyArgsFail())
            this.message = object.toString() + '.' + method + '() should have been called with ' + print.apply(this, this.args) + '; got ' + print.apply(this, this.failingArgs())
          else 
            this.passed = true
      ...
      extend(this, {
        ...
        runDeferredAssertions : function() {
          each(this.assertions, function(assertion){
            if (assertion.defer) {
              assertion.exec()
              assertion.passed ? JSpec.stats.passes++ : JSpec.stats.failures++
            }
          })
        },
    ...
    expect : function(actual) {
      assert = function(matcher, args, negate) {
        ...
        // TODO: remove or abstract deferred assertions
        if (matcher == JSpec.matchers.receive)
          assertion.exec()
        else {
  ...
  addMatchers({
    ...
    receive : function(actual, method, times) {
