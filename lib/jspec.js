...
(function(){
  ...
  var JSpec = {
    ...
    preProcess : function(input) {
      // Allow __END__
      // input = input.replace(/__END__.*/, '')
      // Allow describe literal
      input = input.replace(/describe (.*?)$/m, 'JSpec.addSuite($1, function(){')
      input = input.replace(/describe (.*?)$/gm, 'this.addSuite($1, function(){')
      // Allow it spec literal
      input = input.replace(/it (.*?)$/gm, 'this.addSpec($1, function(){')
      // Allow before / after hooks
      input = input.replace(/^(?: *)(before_each|after_each|before|after)(?= |\n|$)/gm, 'this.addHook("$1", function(){')
      // Allow end }) literal
      input = input.replace(/end(?= |\n|$)/gm, '});')
      // Allow -{ closure literal
      input = input.replace(/-{/g, 'function(){')
      // Allow inclusive range literal n..n
      input = input.replace(/(\d+)\.\.(\d+)/g, function(_, a, b){ return range(a, b) })
      // Allow . this. literal
      input = input.replace(/([\s\(]+)\./gm, '$1this.')
      // Allow optional parens for matchers
      input = input.replace(/\.should([_\.]not)?[_\.](\w+)(?: |$)(.*)$/gm, '.should$1_$2($3)')
      // Convert to non-polluting match() invocation
      input = input.replace(/(.+?)\.(should(?:[_\.]not)?)[_\.](\w+)\((.*)\)$/gm, 'JSpec.match($1, "$2", "$3", [$4]);')
      
      // TODO: fix issue with commenting out assertions (and literals such as -{ )
      // TODO: refactor all this with chaining
      // TODO: re-implement __END__
      // TODO: make sure all grammar features are working
      // TODO: fix running of one suite via suite? with nested support
      return input
    },
