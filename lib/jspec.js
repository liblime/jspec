...
(function(){
  ...
  JSpec = {
     ...
     * this.match('test', 'should', 'be_a', [String])
     // TODO: JSpec.match('test', 'should', 'eql', ['test']) -> expect('test').to('eql', 'test')
     *
     * @param  {object} actual
    ...
    expect : function(actual) {
      assert = function(name, expected, negate) {
      ...
      to = function(name, expected) {
        return assert(name, expected, false)
      }
      
      not_to = function(name, expected) {
        return assert(name, expected, true)
      }
    ...
    preprocess : function(input) {
      return input.
        ...
        replace(/([\/ ]*)(.+?)\.(should(?:[_\.]not)?)[_\.](\w+)\((.*)\)$/gm, '$1 JSpec.expect($2).$3("$4", [$5])').
    ...
    exec : function(file) {
      console.log(this.preprocess(this.load(file)));
