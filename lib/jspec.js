...
(function(){
  ...
  JSpec = {
    ...
    /**
     * Check if _value_ is false. For use as a
    ...
    run : function(options) {
      if (any(hook('running'), isFalse)) return
    ...
    load : function(file) {
      if (any(hook('loading', file), isFalse)) return
      if (this.hasXhr()) {
        var request = this.xhr()
    ...
    exec : function(file) {
      if (any(hook('executing', file), isFalse)) return
  ...
  var main = this
  var find = JSpec.any
  var utils = 'isFalse stub hook destub map any last fail range each option inject select error escape \
