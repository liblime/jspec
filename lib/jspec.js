...
(function(){
  ...
  JSpec = {
     ...
     * 
     * When _url_ is not present the following is used:
    ...
    reportToServer : function(url) {
      url = url || 'http://' + window.location.host + '/results'
      if (any(hook('reportingToServer', url), haveStopped)) return
      JSpec.post(url, 'passes=' + JSpec.stats.passes + '&failures=' + JSpec.stats.failures)
