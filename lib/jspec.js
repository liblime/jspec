...
(function(){
  ...
  var JSpec = {
    ...
    preProcessBody : function(body) {
      ...
      body = body.replace(/\.should([_\.]not)?[_\.](\w+)(?: |$)(.*)$/gm, '.should$1_$2($3)')
      // Convert to non-polluting match() invocation
      body = body.replace(/(.+?)\.(should(?:[_\.]not)?)[_\.](\w+)\((.*)\)$/gm, 'JSpec.match($1, "$2", "$3", [$4])')
    ...
    parse : function(input) {
      ...
      var token, describe, spec, capture, body = [], describe_stack = []
      ...
      while (tokens.length) {
        ...
        switch (token) {
          case 'end':
            if (describe !== null) describe = describe_stack.join(' ')
            
            // suite block
            if (describing && !this.suites[describe]) {
              this.suites[describe] = new JSpec.Suite(describe)
            }
            // specification block
            if (specing) {
