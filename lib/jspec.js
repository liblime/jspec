    ...
    formatters : {
      ...
      DOM : function(results, options) {
        ...
        results.each(results.suites, function(description, suite){
          if (suite.ran) {
            markup += '<tr><td class="description" colspan="3">' + description + '</td></tr>'
            results.each(suite.specs, function(spec){
              ...
              if (spec.requiresImplementation()) { 
                ...
                markup += '<td class="pass">' + spec.description+ '</td><td></td><td>' + assertionCount + '</td>'
              }
              else {
                markup += '<td class="fail">' + spec.description + '<td>' + spec.failure().message + '</td>' + '</td><td>' + assertionCount + '</td>' 
              }
            })
            markup += '</tr>'
          }
