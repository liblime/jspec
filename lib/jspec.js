...
var JSpec = {
  ...
  DOMFormatter : function(results, options) {
    ...
    results.each(results.suites, function(description, suite){
      ++suiteCount
      if (suite.ran) {
        markup += '<div id="suite-' + suiteCount + '" class="suite"><h2>' + description + '</h2>'
        results.each(suite.specs, function(spec){
          ++specCount
          var id = 'id="spec-' + specCount + '"'
          var assertionCount = ' (<span class="assertion-count">' + spec.assertions.length + '</span>)'
          if (spec.requiresImplementation()) { 
            markup += '<p ' + id + ' class="requires-implementation">' + spec.description + '</p>'
          }
          else if (spec.passed()) {
            markup += '<p ' + id + ' class="pass">' + spec.description + assertionCount + '</p>'
          }
          else {
            markup += '<p ' + id + ' class="fail">' + spec.description + assertionCount + ' <em>' + spec.failure().message + '</em>' + '</p>' 
          }
        })
        markup += '</div>'
      }
