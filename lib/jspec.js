...
(function(){
  ...
  var JSpec = {
    ...
    preProcessBody : function(body) {
      ...
      body = body.replace(/(\d+)\.\.(\d+)/g, function(s, start, end){
        start = parseInt(start), end = parseInt(end)
        s = '[' + start
        if (end > start) while (++start <= end) s += ',' + start
        else while (--start >= end) s += ',' + start
        return s + ']'
      })
      // Allow . this. literal
  ...
  addMatchers({
    ...
    be_within          : "actual >= expected[0] && actual <= expected[expected.length-1]",
    have_length_within : "actual.length >= expected[0] && actual.length <= expected[expected.length-1]",
