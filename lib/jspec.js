...
(function(){
  ...
  JSpec = {
    ...
    include : function(object) {
      this.modules.push(object.constructor == JSpec.Module ? object : new JSpec.Module(object))
    },
    
    /**
     * Add a module hook _name_, which is immediately
     * called per module with the _args_ given.
     *
     * @param  {name} string
     * @param  {mixed} args ...
     * @api public
    ...
    hook : function(name, args) {
      each(this.modules, function(module){
        module.hook(name, argumentsToArray(arguments))
      })
    },
    ...
    report : function() {
      hook('reporting', JSpec, JSpec.options)
    ...
    runSuite : function(suite) {
      ...
      suite.ran = true
      suite.hook('before'), hook('before_suite', suite)
      each(suite.specs, function(spec) {
        suite.hook('before_each'), hook('before_spec', spec)
        JSpec.runSpec(spec)
        suite.hook('after_each'), hook('after_spec', spec)
      })
      if (suite.hasSuites()) {
      ...
      suite.hook('after'), hook('after_suite', suite)
    ...
    reportToServer : function(url) {
      hook('reportingToServer', url, JSpec)
  ...
  var main = this
  var find = JSpec.any
  var utils = 'stub hook destub map any last fail range each option inject select error escape \
