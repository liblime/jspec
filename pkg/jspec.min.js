...
var JSpec={version:'0.6.1',main:this,suites:{},stats:{specs:0,assertions:0,failures:0,passes:0},matchers:{be:"alias eql",equal:"===",be_greater_than:">",be_less_than:"<",be_at_least:">=",be_at_most:"<=",be_a:"actual.constructor == expected",be_an:"alias be_a",be_null:"actual == null",be_empty:"actual.length == 0",be_true:"actual == true",be_false:"actual == false",be_type:"typeof actual == expected",match:"typeof actual == 'string' ? actual.match(expected) : false",have_length:"actual.length == expected",respond_to:"typeof actual[expected] == 'function'",eql:{match:function(expected,actual){if(actual.constructor==Array||actual.constructor==Object)return this.hash(actual)==this.hash(expected)
else return actual==expected}},include:{match:function(expected,actual){if(typeof actual=='string')return actual.match(expected)
...
function print(object){if(object==undefined)return''
...
this.match=function(){if(expected!=null)expected=expected.valueOf()
...
return this}},DOMFormatter:function(results,options){var markup='',report=document.getElementById(options.reportToId||'jspec')
...
this.requiresImplementation=function(){return this.assertions.length==0}},hash:function(object){var hash=''
this.each(object,function(key,value){if(value.constructor==Array||value.constructor==Object)hash+=this.hash(value)
else hash+=key.toString()+value.toString()})
return hash},match:function(actual,negate,name,expected){if(typeof negate=='string')negate=negate=='should'?false:true
...
catch(e){this.throw(errorMessage,e)}},preProcessBody:function(body){body=body.replace('-{','function(){')
...
return input.match(regexp)},report:function(options){this.formatter?new JSpec.formatter(this,options||{}):new JSpec.DOMFormatter(this,options||{})
...
catch(e){this.throw('depends on '+dependency+' '+(message||''))}},throw:function(message,error){throw'jspec: '+message+(error?error.message+'; in '+error.fileName+' on line '+error.lineNumber:'')},eval:function(input){return this.parse(input)},load:function(file){if('XMLHttpRequest'in this.main){var request=new XMLHttpRequest
request.open('GET',file,false)
request.send(null)
if(request.readyState==4)
return request.responseText}
else if('load'in this.main){load(file)}
else{this.throw('cannot load '+file)}},exec:function(file){return this.eval(this.load(file))}}
