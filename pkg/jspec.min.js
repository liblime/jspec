...
(function(){var JSpec={version:'0.7.0',main:this,suites:{},matchers:{},stats:{specs:0,assertions:0,failures:0,passes:0},options:{profile:false},defaultContext:{sandbox:function(name){var sandbox=document.createElement('div')
...
return this}},formatters:{DOM:function(results,options){var id=options.reportToId||'jspec'
...
results.each(suite.specs,function(spec){var assertionCount=spec.assertions.length+':'
...
console.groupEnd()}})}},Suite:function(description){this.specs=[],this.hooks={},this.description=description,this.ran=false
...
catch(e){error('depends on '+dependency+' '+(message||''))}},error:function(message,error){throw'jspec: '+message+(error?error.message+'; in '+error.fileName+' on line '+error.lineNumber:'')},eval:function(input){return this.parse(input)},load:function(file){if('XMLHttpRequest'in this.main){var request=new XMLHttpRequest
