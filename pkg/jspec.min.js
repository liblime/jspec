...
(function(){JSpec={version:'0.9.6',main:this,suites:[],matchers:{},stats:{specs:0,assertions:0,failures:0,passes:0},options:{profile:false},defaultContext:{sandbox:function(name){sandbox=document.createElement('div')
...
return this}},formatters:{DOM:function(results,options){id=options.reportToId||'jspec'
...
function renderSuite(suite){displaySuite=options.failuresOnly?suite.ran&&!suite.passed():suite.ran
...
if(spec.requiresImplementation()&&!options.failuresOnly){markup+='<td class="requires-implementation" colspan="2">'+spec.description+'</td>'}
else if(spec.passed()&&!options.failuresOnly){markup+='<td class="pass">'+spec.description+'</td><td>'+spec.assertionsGraph()+'</td>'}
...
console.groupEnd()}})}},Suite:function(description,body){this.body=body,this.suites=[],this.specs=[]
...
this.assertionsGraph=function(){return map(this.assertions,function(assertion){return assertion.passed?'<span class="assertion passed"></span>':'<span class="assertion failed"></span>'}).join('')}},hash:function(object){serialize=function(prefix){return inject(object,prefix+':',function(buffer,key,value){return buffer+=hash(value)})}
...
return matcher.result},each:function(object,callback){for(key in object){if(object.hasOwnProperty(key))
...
return'['+values+']'},report:function(options){this.formatter?new this.formatter(this,options||{}):new this.formatters.DOM(this,options||{})
