...
(function(){var JSpec={version:'1.1.7',suites:[],allSuites:[],matchers:{},stats:{specs:0,assertions:0,failures:0,passes:0,specsFinished:0,suitesFinished:0},options:{profile:false},defaultContext:{an_instance_of:function(constructor){return{an_instance_of:constructor}},wait:function(n,unit){JSpec.currentSpec.wait={'second':n*1000,'seconds':n*1000}[unit]||n}},formatters:{DOM:function(results,options){id=option('reportToId')||'jspec'
...
each(suite.specs,function(spec){assertionCount=spec.assertions.length+':'
if(spec.requiresImplementation())
...
renderSuites=function(suites){each(suites,function(suite){renderSuite(suite)
if(suite.hasSuites())renderSuites(suite.suites)})}
renderSuites(results.suites)}},Assertion:function(matcher,actual,expected,negate){extend(this,{message:'',passed:false,actual:actual,negate:negate,matcher:matcher,expected:expected,report:function(){this.passed?JSpec.stats.passes++:JSpec.stats.failures++
return this},run:function(){expected.unshift(actual==null?null:actual.valueOf())
...
return this}})},ProxyAssertion:function(object,method,times){var self=this
var old=object[method]
object[method]=function(){args=argumentsToArray(arguments)
result=old.apply(object,args)
self.calls.push({args:args,result:result})
return result}
this.times={'once':1,'twice':2}[times]||times||1
extend(this,{calls:[],message:'',defer:true,passed:false,object:object,method:method,and_return:function(result){this.expectedResult=result
return this},with_args:function(){this.expectedArgs=argumentsToArray(arguments)
return this},anyResultsFail:function(){return any(this.calls,function(call){return self.expectedResult.an_instance_of?call.result.constructor!=self.expectedResult.an_instance_of:hash(self.expectedResult)!=hash(call.result)})},failingResult:function(){return this.anyResultsFail().result},anyArgsFail:function(){return any(this.calls,function(call){return any(self.expectedArgs,function(i,arg){return arg.an_instance_of?call.args[i].constructor!=arg.an_instance_of:hash(arg)!=hash(call.args[i])})})},failingArgs:function(){return this.anyArgsFail().args},report:function(){this.passed?JSpec.stats.passes++:JSpec.stats.failures++
return this},run:function(){methodString='expected '+object.toString()+'.'+method+'()'
times=function(n){return n>2?n+' times':{1:'once',2:'twice'}[n]}
if(this.calls.length<this.times)
this.message=methodString+' to be called '+times(this.times)+', but '+(this.calls.length==0?' was not called':' was called '+times(this.calls.length))
if(this.expectedResult&&this.anyResultsFail())
this.message=methodString+' to return '+print(this.expectedResult)+' but got '+print(this.failingResult())
if(this.expectedArgs&&this.anyArgsFail())
this.message=methodString+' to be called with '+print.apply(this,this.expectedArgs)+' but was called with '+print.apply(this,this.failingArgs())
...
each(this.hooks[hook],function(body){JSpec.evalBody(body,"Error in hook '"+hook+"', suite '"+self.description+"': ")})},hasSuites:function(){return this.suites.length},hasSpecs:function(){return this.specs.length},passed:function(){return!any(this.specs,function(spec){return!spec.passed()})}})},Spec:function(description,body){extend(this,{body:body,description:description,assertions:[],runDeferredAssertions:function(){each(this.assertions,function(assertion){if(assertion.defer)assertion.run().report()})},failure:function(){return find(this.assertions,function(assertion){return!assertion.passed})},failures:function(){return select(this.assertions,function(assertion){return!assertion.passed})},passed:function(){return!this.failure()},requiresImplementation:function(){return this.assertions.length==0},assertionsGraph:function(){return map(this.assertions,function(assertion){return'<span class="assertion '+(assertion.passed?'passed':'failed')+'"></span>'}).join('')}})},DSLs:{snake:{expect:function(actual){return JSpec.expect(actual)},describe:function(description,body){return JSpec.currentSuite.addSuite(description,body)},it:function(description,body){return JSpec.currentSuite.addSpec(description,body)},before:function(body){return JSpec.currentSuite.addHook('before',body)},after:function(body){return JSpec.currentSuite.addHook('after',body)},before_each:function(body){return JSpec.currentSuite.addHook('before_each',body)},after_each:function(body){return JSpec.currentSuite.addHook('after_each',body)},should_behave_like:function(description){return JSpec.shareBehaviorsOf(description)}}},findSuite:function(description){return find(this.allSuites,function(suite){return suite.name==description||suite.description==description})},shareBehaviorsOf:function(description){if(suite=this.findSuite(description))this.copySpecs(suite,this.currentSuite)
else throw'failed to share behaviors. '+print(description)+' is not a valid Suite name'},copySpecs:function(fromSuite,toSuite){each(fromSuite.specs,function(spec){toSuite.specs.push(spec)})},argumentsToArray:function(arguments,offset){args=[]
for(i=0;i<arguments.length;i++)args.push(arguments[i])
return args.slice(offset||0)},color:function(string,color){return"\u001B["+{bold:1,black:30,red:31,green:32,yellow:33,blue:34,magenta:35,cyan:36,white:37}[color]+'m'+string+"\u001B[0m"},defaultMatcherMessage:function(actual,expected,negate,name){return'expected '+print(actual)+' to '+
(negate?'not ':'')+
...
default:return body}},option:function(key){return(value=query(key))!==null?value:JSpec.options[key]||null},hash:function(object){if(object==undefined)return'undefined'
serialize=function(prefix){return inject(object,prefix+':',function(buffer,key,value){return buffer+=hash(value)})}
...
default:return object.toString()}},last:function(array){return array[array.length-1]},print:function(object){if(arguments.length>1){return map(argumentsToArray(arguments),function(arg){return print(arg)}).join(', ')}
...
case Array:return inject(object,'[',function(b,v){return b+', '+print(v)}).replace('[,','[')+' ]'
case Object:return inject(object,'{',function(b,k,v){return b+', '+print(k)+' : '+print(v)}).replace('{,','{')+' }'
default:return escape(object.toString())}},escape:function(html){return html.toString().replace(/&/gmi,'&amp;').replace(/"/gmi,'&quot;').replace(/>/gmi,'&gt;').replace(/</gmi,'&lt;')},does:function(actual,matcher,expected){assertion=new JSpec.Assertion(JSpec.matchers[matcher],actual,argumentsToArray(arguments,2))
return assertion.run().result},expect:function(actual){assert=function(matcher,args,negate){expected=[]
...
return{to:to,should:to,not_to:not_to,should_not:not_to}},strip:function(string,chars){return string.replace(new RegExp('['+(chars||'\\s')+']*$'),'').replace(new RegExp('^['+(chars||'\\s')+']*'),'')},callIterator:function(callback,a,b){return callback.length==1?callback(b):callback(a,b)},extend:function(object,other){each(other,function(property,value){object[property]=value})},each:function(object,callback){if(typeof object=='string')object=object.split(' ')
for(key in object)
if(object.hasOwnProperty(key))
callIterator(callback,key,object[key])},inject:function(object,memo,callback){each(object,function(key,value){memo=(callback.length==2?callback(memo,value):callback(memo,key,value))||memo})
return memo},map:function(object,callback){return inject(object,[],function(memo,key,value){memo.push(callIterator(callback,key,value))})},any:function(object,callback){return inject(object,null,function(state,key,value){return state?state:callIterator(callback,key,value)?value:state})},select:function(object,callback){return inject(object,[],function(memo,key,value){if(callIterator(callback,key,value))
memo.push(value)})},addMatchers:function(matchers){each(matchers,function(name,body){JSpec.addMatcher(name,body)})},addMatcher:function(name,body){this.matchers[name]=this.normalizeMatcherMessage(this.normalizeMatcherBody(body))
this.matchers[name].name=name},describe:function(description,body){suite=new JSpec.Suite(description,body)
this.allSuites.push(suite)
this.suites.push(suite)},contentsOf:function(body){return body.toString().match(/^[^\{]*{((.*\n*)*)}/m)[1]},evalBody:function(body,errorMessage){dsl=this.DSL||this.DSLs.snake
...
catch(e){error(errorMessage,e)}},preprocess:function(input){return input.replace(/describe (.*?)$/gm,'describe($1, function(){').replace(/ it (.*?)$/gm,' it($1, function(){').replace(/^(?: *)(before_each|after_each|before|after)(?= |\n|$)/gm,'JSpec.currentSuite.addHook("$1", function(){').replace(/end(?= |\n|$)/gm,'});').replace(/-\{/g,'function(){').replace(/(\d+)\.\.(\d+)/g,function(_,a,b){return range(a,b)}).replace(/\.should([_\.]not)?[_\.](\w+)(?: |$)(.*)$/gm,'.should$1_$2($3)').replace(/([\/ ]*)(.+?)\.(should(?:[_\.]not)?)[_\.](\w+)\((.*)\)$/gm,'$1 expect($2).$3($4, $5)').replace(/, \)/gm,')').replace(/should\.not/gm,'should_not')},range:function(start,end){current=parseInt(start),end=parseInt(end),values=[current]
if(end>current)while(++current<=end)values.push(current)
else while(--current>=end)values.push(current)
return'['+values+']'},whenFinished:function(callback){if(this.stats.specsFinished>=this.stats.specs)callback()
else setTimeout(function(){JSpec.whenFinished(callback)},50)},report:function(){this.whenFinished(function(){JSpec.options.formatter?new JSpec.options.formatter(JSpec,JSpec.options):new JSpec.formatters.DOM(JSpec,JSpec.options)})},run:function(options){if(options)extend(this.options,options)
if(option('profile'))console.group('Profile')
each(this.suites,function(suite){JSpec.runSuite(suite)})
if(option('profile'))console.groupEnd()
return this},whenCurrentSpecIsFinished:function(callback){if(this.currentSpec&&this.currentSpec.wait)
setTimeout(callback,this.currentSpec.wait)
else callback()},runSuite:function(suite){this.currentSuite=suite
this.evalBody(suite.body)
suite.ran=true
suite.hook('before')
each(suite.specs,function(spec){JSpec.whenCurrentSpecIsFinished(function(){suite.hook('before_each')
JSpec.runSpec(spec)
suite.hook('after_each')})})
if(suite.hasSuites()){each(suite.suites,function(suite){JSpec.runSuite(suite)})}
suite.hook('after')
this.stats.suitesFinished++},fail:function(message){JSpec.currentSpec.assertions.push({passed:false,message:message})
...
request.send(data)},reportToServer:function(){this.whenFinished(function(){JSpec.post('http://localhost:4444','passes='+JSpec.stats.passes+'&failures='+JSpec.stats.failures)
if('close'in main)main.close()})},xhr:function(){return'ActiveXObject'in main?new ActiveXObject("Microsoft.XMLHTTP"):new XMLHttpRequest()},hasXhr:function(){return'XMLHttpRequest'in main||'ActiveXObject'in main},load:function(file){if(this.hasXhr()){request=this.xhr()
...
return length>=range.shift()&&length<=range.pop()},have_prop:function(actual,property,value){return actual[property]==null||actual[property]instanceof Function?false:value==null?true:does(actual[property],'eql',value)},have_property:function(actual,property,value){return actual[property]==null||actual[property]instanceof Function?false:value==null?true:value===actual[property]}})
this.JSpec=JSpec})()